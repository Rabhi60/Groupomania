...
<div class="container-fluid">
    ...
    <section class="col-10 mx-auto  cardMessage " >
      <b-card :title="messageUnique.title" :img-src="messageUnique.attachment" img-alt="Image" img-top :header="'by ' + User.username" tag="article"  bg-variant="dark" text-variant="white" class="text-center mb-2 col-12 mx-auto ">
        ...
        <b-button variant="success" class="mx-1 my-1">
         ...
         <b-button variant="danger " class="mx-1 my-1">
      ...
      <router-link :to='`/Home/ModifyMessage/${id}`'>
      ...
      <b-modal id="modal-prevent-closing" ref="modal" title="Nouveau commentaire" @show="resetModal" @hidden="resetModal" @ok="handleOk">
        <form ref="form" @submit.stop.prevent="handleSubmit">
          <b-form-group :state="commentState" label="Commentaire" label-for="comment" invalid-feedback="Comment is required">
            <b-form-input id="comment" v-model="comment" :state="commentState" required ></b-form-input>
          </b-form-group>
        </form>
      </b-modal> 
    </section>
...
export default {
  ...
  methods: {
    ...
    onSubmit(evt) {
      evt.preventDefault()
      const self = this;
      axios.delete(`http://localhost:3000/api/messages/delete/${this.submit.id}`,
        { headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Authorization': `Barer ${sessionToken}`
  }})
    ...
    deconnexion: function(){
     ...
     checkFormValidity() {
      ...
      handleSubmit() {
        ...
        axios.get(`http://localhost:3000/api/message/newComment/`,{ userId: this.userId, messageId: this.id, },
      {headers: {
      'Content-Type': 'application/x-www-form-urlencoded',
      'Authorization': `Barer ${sessionToken}`
    }})
